<script context="module">
  export async function load({page}) {
    const res = await fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=04c35731a5ee918f014970082a0088b1&language=en-US&page=1`);
...
export let totalPage;
  ...
  const load_more = async () => {
		currentPage += 1;
		const res = await fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=04c35731a5ee918f014970082a0088b1&language=en-US&page=${currentPage}`);
		const raw_movie_data = await res.json();
...
<div class="conatiner">
	...
	<Title>
		Top Rated on <span class="imdb">IMDB</span>
	</Title>
